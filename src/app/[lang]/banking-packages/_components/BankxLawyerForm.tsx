    ...
    <div className='flex flex-col gap-6'>
      <div className='flex flex-col gap-2'>
        <div className="flex items-start gap-2">
          ...
          <label htmlFor="bankAndLawyer.bankAppointment" className="text-xs cursor-pointer">Do you require a bank appointment? (Minimum deposit is €{getMinimumDeposit.toLocaleString()})</label>
        </div>
        ...
        <div className="flex items-start gap-2">
          ...
          <label htmlFor="bankAndLawyer.lawyerAppointment" className="text-xs cursor-pointer">Do you require a lawyers appointment?</label>
        </div>
      </div>
      <div className='flex flex-col gap-2'>
        <p className='font-medium'>Travel Details</p>
        <div className='flex flex-col gap-4'>
          <Input type='number' required max={5} min={0} title='How many people are you traveling with?' name='travelInfo.numberOfPeople' value={values.travelInfo.numberOfPeople} onChange={handleChangeInNumberOfPeople} />
          <div className='grid grid-cols-2 gap-3'>
            <Input required type="datetime-local" name='travelInfo.arrivalDate' title='Arrival Date + Time' value={values.travelInfo.arrivalDate} onChange={handleChange} />
            <Input required type="datetime-local" name='travelInfo.departureDate' title='Departure Date + Time' value={values.travelInfo.departureDate} onChange={handleChange} />
          </div>
          <div className="flex items-start gap-2">
            ...
            <label htmlFor="travelInfo.requireRentalCar" className="text-xs cursor-pointer">Do you require a rental car? (Minimum rental period is 3 days)</label>
          </div>
