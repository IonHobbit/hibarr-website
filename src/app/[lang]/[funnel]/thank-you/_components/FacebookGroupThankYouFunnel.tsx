...
export default async function FacebookGroupThankYouFunnel() {

  const isBookedConsultation = storage.get<boolean>(StorageKey.BOOKED_CONSULTATION);

  const [title, subtitle, nextStepsTitle, facebookGroupConfirmation, nextWebinar] = await translateBatch(['You are in!', 'We will send you an email shortly to confirm your Facebook Group membership.', 'What happens next', 'Check your email for Facebook Group confirmation', 'You will be notified when the next webinar is scheduled.']);

  const [bookConsultation, returnHome] = await translateBatch(['Book a consultation', 'Return home']);
      ...
      <div className='flex flex-col items-center gap-4 w-full px-5 pb-5'>
        <div className='w-full overflow-hidden rounded flex flex-col gap-2 border-[0.5px]'>
          ...
          <div className='flex flex-col gap-2 px-4 pb-4'>
            <p className='text-xs'>- {facebookGroupConfirmation.text}</p>
            <p className='text-xs'>- {nextWebinar.text}</p>
          </div>
        </div>
