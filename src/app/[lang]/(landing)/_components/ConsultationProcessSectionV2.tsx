...
export default function ConsultationProcessSection({ data }: ConsultationProcessSection) {
  return (
    <section id='consultation-process' className='section'>
      <div className="bg-primary rounded-lg p-6 md:p-9 flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 md:flex flex-wrap justify-center gap-6">
          {data?.steps?.map((step, index) => (
            <div
              key={index}
              className="group overflow-hidden cursor-default flex flex-col gap-4 rounded-xl p-4 py-5 basis-full bg-white md:basis-[31.5%] md:nth-last-[-n+2]:basis-[48%] transition-all duration-300">
