...
function FileInput({ className, title, error, onUpload, ...props }: React.ComponentProps<"input"> & { error?: string, onUpload: (result: string) => void }) {
    ...
    <div className="relative flex flex-col items-start gap-1">
      ...
      <div className="relative border-input flex items-center justify-between gap-4 min-h-10 w-full min-w-0 border bg-transparent file:border-0 focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive rounded px-3 py-1.5">
        ...
        <input
          ...
          data-slot="input"
          className={cn(
            "hidden file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30 text-base transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:cursor-pointer  file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
            className
          )}
